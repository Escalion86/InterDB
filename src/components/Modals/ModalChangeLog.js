...
const data = [
  {
    ...
    date: '07.12.2020',
    updated: [
      'Полностью переработан и существенно оптимизирован фильтр по месяцам (особенно заметно при большом количестве данных)',
      'В графике "Чистая прибыль по месяцам" месяца с незавершенными событиями теперь отображаются пустым кружком',
      'Теперь разрешение доступа приложению к Google Календарю запрашивается только при включении синхронизации с календарем',
    ],
    other: ['Подготовления к будущему модулю синхронизации с облаком'],
  },
  {
    version: '0.5.0',
    date: '22.11.2020',
